...
class UncertaintyWeightedLoss(nn.Module):
    """Kendall, Gal & Cipolla, CVPR 2018: learn task weights via homoscedastic
    uncertainty instead of a hand-tuned lambda (the S2 failure mode).

    ``L = e^{-s_seg} L_seg + s_seg + e^{-s_cls} L_cls + s_cls``.
    """

    is_uncertainty = True

    def __init__(self, seg_loss: nn.Module):
        super().__init__()
        self.seg_loss = seg_loss
        self.s_seg = nn.Parameter(torch.zeros(()))
        self.s_cls = nn.Parameter(torch.zeros(()))
        self.bce = nn.BCEWithLogitsLoss()

    def seg(self, logits, target):
        return self.seg_loss(logits, target)

    def combine(self, l_seg, cls_logits, cls_target):
        l_cls = self.bce(cls_logits.reshape(cls_target.shape), cls_target)
        return (torch.exp(-self.s_seg) * l_seg + self.s_seg
                + torch.exp(-self.s_cls) * l_cls + self.s_cls)
...
def create_loss(cfg: Dict) -> nn.Module:
    ...
    When ``loss.uncertainty_weight`` is set the caller wraps this in
    ``UncertaintyWeightedLoss`` and trains the scan-level head via
    ``model.classify`` (not patch occupancy). Skip MultiTaskLoss in that
    case so the two cls objectives cannot fight.

    Otherwise, ``model.cls_head`` wraps the segmentation loss in

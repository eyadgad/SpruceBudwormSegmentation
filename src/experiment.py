...
def _loaders(cfg, manifest, norm_stats):
    ...
    cls_loader = None
    if bool(cfg.get("loss", {}).get("uncertainty_weight", False)):
        from .classify import ScanClsDataset
        cls_ds = ScanClsDataset(cfg, manifest, "train", norm_stats, train=True)
        cls_bs = int(cfg["train"].get("cls_batch_size", 4))
        cls_loader = DataLoader(cls_ds, batch_size=cls_bs, shuffle=True, drop_last=True, **common)
    return train_loader, val_loader, sampler, cls_loader
...
def run_experiment(cfg: Dict, manifest, norm_stats, device, verbose=True) -> Dict:
    ...
    if init_path and not ckpt.resume_path(ckpt_dir, name).exists():
        from pathlib import Path as _Path
        init_state = ckpt.load_checkpoint(_Path(init_path), device)
    ...
    train_loader, val_loader, train_sampler, cls_loader = _loaders(cfg, manifest, norm_stats)
    ...
    if not training_complete:
        ...
        for epoch in range(start_epoch, epochs):
            ...
            # Divergence stop: a non-finite forward skips every batch
            # (train_loss==0) and is unrecoverable. A single epoch of
            # all-negative predictions (val_dice==0) is not that — S1 died
            # at epoch 31 on a conservative collapse while train_loss was
            # still finite. Early-stop patience governs those cases.
            diverged = (tr_loss == 0.0)
            if diverged:
    ...
    if result.get("calibrated_cls_threshold") is not None:
        sa = tm.get("scan_auroc")
        log.info(f"  hard gate       | cls_t={result['calibrated_cls_threshold']:.2f} "
                 f"scan_auroc={float('nan') if sa is None else float(sa):.4f} "
                 f"scan_f1={tm.get('scan_f1')}")
    if result.get("val_ungated"):
        ug = result["val_ungated"]
        log.info(f"  ungated         | dice={ug['dice']:.4f} "
                 f"bg_fp_rate={ug.get('bg_fp_rate', float('nan')):.4f}")
    return result

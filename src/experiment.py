...
from .dataset import RadarPatchDataset, SceneGroupedSampler
...
def _loaders(cfg, manifest, norm_stats):
    ...
    sampler = SceneGroupedSampler(train_ds, shuffle=True, seed=cfg["split"]["seed"])
...
def run_experiment(cfg: Dict, manifest, norm_stats, device, verbose=True) -> Dict:
    ...
    log.info(f"evaluating best checkpoint (ep{best_epoch}, val_dice={best_dice:.4f}) on full test scenes ...")
    best_t = engine.calibrate_threshold(model, val_rows, cfg, norm_stats, device,
                                        thresholds, max_scenes=calib_max)
    test_metrics = engine.evaluate_full_scene(model, test_rows, cfg, norm_stats, device,
                                              threshold=best_t, tta=tta)
    ...
    log.info(f"  TEST full-scene | dice(macro)={tm['dice']:.4f} dice(micro)={tm.get('dice_micro', float('nan')):.4f} "

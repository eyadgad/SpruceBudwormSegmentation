...
def build_manifest(cfg: Dict) -> pd.DataFrame:
    ...
    balanced = bool(neg_cfg.get("balanced", False))
    if neg_cfg.get("include", True):
        ...
        for year in sorted(pos_per_year):
            ...
            if not negs:
                continue
            # balanced: take every unused negative so each split can be
            # downsampled to n_pos after nights are assigned. ratio mode
            # keeps the historical 0.3 subsample.
            if balanced:
                take = negs
            else:
                budget = int(round(float(neg_cfg.get("ratio", 0.3)) * pos_per_year[year]))
                if budget <= 0:
                    continue
                sel = rng.choice(len(negs), size=min(budget, len(negs)), replace=False)
                take = [negs[i] for i in sorted(sel)]
            for ts in take:
    ...
    if balanced:
        df = _balance_negatives_per_split(df, seed)
        if mode == "night":
            nights.verify_no_leakage(df)
    return df


def _balance_negatives_per_split(df: pd.DataFrame, seed: int) -> pd.DataFrame:
    """Downsample quiet scans so each split has n_neg == n_pos (or all negs if fewer).

    Nights stay intact as groups: we only drop whole negative *scans*, never
    move a night across splits. The archive has 1500 negatives vs 1579
    positives, so a split can still come up a few quiet scans short.
    """
    ...
    for split in ("train", "val", "test"):
        ...
        n_pos = int(len(pos))
        if len(neg) > n_pos > 0:
            chosen = rng.choice(neg.index.to_numpy(), size=n_pos, replace=False)
            neg = neg.loc[np.sort(chosen)]
        parts.extend([pos, neg])
    out = pd.concat(parts, ignore_index=True)
    return out.sort_values(["split", "timestamp"]).reset_index(drop=True)
